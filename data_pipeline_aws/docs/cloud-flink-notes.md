...
| Kafka | Docker KRaft | Amazon MSK |
| S3 Storage | LocalStack | AWS S3 |
| Iceberg Catalog | Hadoop | AWS Glue (recommended) |
| Credentials | Static (test/test) | IAM roles |
...
ICEBERG_WAREHOUSE=s3://analytics-data/warehouse

# Cloud
KAFKA_BOOTSTRAP_SERVERS=b-1.msk-cluster.xxx.kafka.us-east-1.amazonaws.com:9092
AWS_ENDPOINT_URL=  # Not needed, uses real AWS
ICEBERG_WAREHOUSE=s3://your-bucket/warehouse
```

### Iceberg Catalog (Glue)

To use AWS Glue catalog instead of Hadoop:
...
# Package Python job
cd flink
zip -r flink-job.zip jobs/ common/ requirements.txt
