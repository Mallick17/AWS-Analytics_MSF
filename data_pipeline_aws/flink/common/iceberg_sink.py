...
class IcebergSinkFactory:
    ...
    def create_catalog_ddl(self) -> str:
        ...
        if is_aws_environment():
            ...
            """
            logger.info(f"Created S3 Tables catalog DDL: {iceberg_config.catalog_name}")
        else:
            # Local Hadoop Catalog (filesystem)
            ddl = f"""
            CREATE CATALOG {iceberg_config.catalog_name} WITH (
                'type' = 'iceberg',
                'catalog-type' = 'hadoop',
            ...
            """
            logger.info(f"Created Hadoop catalog DDL: {iceberg_config.catalog_name}")
