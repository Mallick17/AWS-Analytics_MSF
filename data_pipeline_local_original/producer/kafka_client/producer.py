...
class KafkaEventProducer:
    ...
    def __init__(
        ...
        # Handle bootstrap_servers being either string or dict
        if isinstance(bootstrap_servers, dict):
            # If dict, try to extract a common key or join values
            if 'servers' in bootstrap_servers:
                bootstrap_servers_str = bootstrap_servers['servers']
            elif 'bootstrap_servers' in bootstrap_servers:
                bootstrap_servers_str = bootstrap_servers['bootstrap_servers']
            else:
                # Fallback: join all values
                bootstrap_servers_str = ','.join(str(v) for v in bootstrap_servers.values())
        else:
            bootstrap_servers_str = str(bootstrap_servers)

        # Fallback to localhost if value is 'None' or empty
        if not bootstrap_servers_str or bootstrap_servers_str == 'None':
            bootstrap_servers_str = 'localhost:9092'
    ...
    def _on_success(self, record_metadata) -> None:
        ...
        try:
            topic = getattr(record_metadata, 'topic', 'unknown')
            partition = getattr(record_metadata, 'partition', 'unknown')
            offset = getattr(record_metadata, 'offset', 'unknown')
            logger.debug(
                f"Delivered to {topic}[{partition}] "
                f"@ offset {offset}"
            )
