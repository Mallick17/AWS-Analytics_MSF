...
if docker compose ps --status running 2>/dev/null | grep -q "kafka\|jobmanager"; then
...
while [ $timeout -gt 0 ]; do
    if docker compose exec -T zookeeper bash -c "cub zk-ready localhost:2181 5" &>/dev/null; then
        echo "✓ Ready"
        break
    fi
    sleep 2
...
echo ""
echo "Run './infra/scripts/health-check.sh' to verify all services."
echo ""
